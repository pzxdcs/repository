...
- (IBAction)like:(UIButton *)sender {
    if ([[loginModel currentLogin]isLogin]) {
        searchResultCell *cell = (searchResultCell *)sender.superview.superview;
        NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
        //NSLog(@"%ld",(long)indexPath.row);
        searchModel *models = self.data[indexPath.row];
        NSString *msgId = models.msgid;
        NSDictionary *params = @{@"c":@"iosapp",@"a":@"collect",@"msgid":msgId,@"userid":[loginModel currentLogin].userId};
        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
        [manager GET:kBaseURL parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            NSLog(@"%@",responseObject);
